...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("google-gemini-cli", "Google Gemini (OAuth)",    "Google Gemini (Cloud Code Assist OAuth)"),
...
def _is_confirmed_kimi_coding_catalog_url(base_url: str) -> bool:
    """Return True only for Kimi Coding's canonical HTTPS model endpoints."""
    try:
        ...
        if _p and _p.auth_type == "api_key" and _p.base_url:
            ...
            if api_key:
                ...
                if live:
                    ...
                    # Kimi Coding Plan's current model ID is the literal ``k3``.
                    # Never advertise it for legacy Moonshot or arbitrary custom
                    # endpoints just because this shared provider profile was used.
                    if normalized == "kimi-coding" and not _is_confirmed_kimi_coding_catalog_url(base_url):
                        curated = [m for m in curated if m.strip().lower() != "k3"]
                    if curated:
