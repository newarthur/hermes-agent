...
def _profile_live_catalog(normalized: str) -> Optional[list[str]]:
    ...
    if normalized == "kimi-coding" and not _is_confirmed_kimi_coding_catalog_url(base_url):
        if live:
            live = [m for m in live if m.strip().lower() != "k3"]
    if not live:
        fallback = list(profile.fallback_models) if profile.fallback_models else None
        if fallback and normalized == "kimi-coding" and not _is_confirmed_kimi_coding_catalog_url(base_url):
            fallback = [m for m in fallback if m.strip().lower() != "k3"]
        return fallback
