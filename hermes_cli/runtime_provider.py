...
from hermes_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
    ...
    resolve_qwen_runtime_credentials, resolve_gemini_oauth_runtime_credentials, resolve_api_key_provider_credentials,
...
_POOL_ENTRY_SIMPLE_MODES: Dict[str, tuple] = {
    ...
    "google-gemini-cli": ("chat_completions", "cloudcode-pa://google"), "openrouter": ("chat_completions", OPENROUTER_BASE_URL),
...
def _gemini_oauth_runtime(provider, requested_provider) -> Optional[Dict[str, Any]]:
    from hermes_cli.auth import resolve_gemini_oauth_runtime_credentials, AuthError
    try:
...
def _ladder_rungs(requested_provider, explicit_api_key, explicit_base_url, target_model):
    ...
    if provider == "google-gemini-cli":
        yield _gemini_oauth_runtime(provider, requested_provider)
